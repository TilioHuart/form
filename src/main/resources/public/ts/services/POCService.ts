...
export const pocService: POCService = {
    async getPublicFormByKey(formKey: string) : Promise<any> {
        try {
            return DataUtils.getData(await http.get(`/formulaire/p/forms/key/${formKey}`));
        } catch (err) {
    ...
    async sendResponses(formKey: string, distributionKey: string, responses: Responses) : Promise<any> {
        try {
            return DataUtils.getData(await http.post(`/formulaire/p/responses/${formKey}/${distributionKey}`, responses.all));
        } catch (err) {
