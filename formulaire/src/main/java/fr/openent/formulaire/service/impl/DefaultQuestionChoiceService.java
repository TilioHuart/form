...
public class DefaultQuestionChoiceService implements QuestionChoiceService {
    ...
    @Override
    public void create(String questionId, JsonObject choice, String locale, Handler<Either<String, JsonObject>> handler) {
        String query = "INSERT INTO " + QUESTION_CHOICE_TABLE + " (question_id, value, position, type, next_section_id, is_custom) " +
                "VALUES (?, ?, ?, ?, ?, ?) RETURNING *;";
        boolean isCustom = choice.getBoolean(IS_CUSTOM, false);
        JsonArray params = new JsonArray()
                .add(questionId)
                .add(isCustom ? I18nHelper.getI18nValue(I18nKeys.OTHER, locale) : choice.getString(VALUE, ""))
    ...
    public void duplicate(int formId, int questionId, int originalQuestionId, Handler<Either<String, JsonObject>> handler) {
        String query = "INSERT INTO " + QUESTION_CHOICE_TABLE + " (question_id, value, position, type, is_custom, next_section_id) " +
                "SELECT ?, value, position, type, is_custom, " +
                "(SELECT id FROM " + SECTION_TABLE + " WHERE original_section_id = qc.next_section_id AND form_id = ?) " +
    ...
    @Override
    public void update(String choiceId, JsonObject choice, String locale, Handler<Either<String, JsonObject>> handler) {
        String query = "UPDATE " + QUESTION_CHOICE_TABLE + " SET value = ?, position = ?, type = ?, next_section_id = ?, is_custom = ? " +
                "WHERE id = ? RETURNING *;";
        boolean isCustom = choice.getBoolean(IS_CUSTOM, false);
        JsonArray params = new JsonArray()
                .add(isCustom ? I18nHelper.getI18nValue(I18nKeys.OTHER, locale) : choice.getString(VALUE, ""))
