...
public class DefaultFormElementService implements FormElementService {
    ...
    public void countFormElements(String formId, Handler<Either<String, JsonObject>> handler) {
        String countQuestions = "SELECT COUNT(*) FROM " + Tables.QUESTION + " WHERE form_id = ? AND section_id IS NULL";
        String countSections = "SELECT COUNT(*) FROM " + Tables.SECTION + " WHERE form_id = ?";
    ...
    public void getTypeAndIdByPosition(String formId, String position, Handler<Either<String, JsonObject>> handler) {
        String getQuestions = "SELECT id, 'question' AS element_type FROM " + Tables.QUESTION + " WHERE form_id = ? AND position = ?";
        String getSections = "SELECT id, 'section' AS element_type FROM " + Tables.SECTION + " WHERE form_id = ? AND position = ?";
    ...
    public void getByTypeAndId(String elementId, String elementType, Handler<Either<String, JsonObject>> handler) {
        String table = elementType.equals("question") ? Tables.QUESTION : Tables.SECTION;
